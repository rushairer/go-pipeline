...
func TestPipeline_ArrayVsSliceMemoryCopy(t *testing.T) {
    ...
    t.Run("slice element shares underlying bytes", func(t *testing.T) {
        ...
        var seen [][]byte
        p := NewStandardPipeline[[]byte](cfg, func(ctx context.Context, batch [][]byte) error {
    ...
    t.Run("array element is value-copied", func(t *testing.T) {
        type Arr8 = [8]byte
        var seen []Arr8
        p := NewStandardPipeline[Arr8](cfg, func(ctx context.Context, batch []Arr8) error {
            // 直接记录 batch（数组元素是值）
...
func TestPipeline_PointerNoDuplicateMemoryButShared(t *testing.T) {
    ...
    var seen [][]byte
    p := NewStandardPipeline[*Large](cfg, func(ctx context.Context, batch []*Large) error {
        for _, v := range batch {
            // 观察当前指针对象的内容（做快照）
    ...
    for i := range large.data { large.data[i] = byte(i) }
